###################################################
# Program Name: ZMONTHLY_ZBGHBILLDTL.py
###################################################
'''
...
cur  = conn.cursor()
#cur.execute("select sap_code,company_name,customer_name2,emp_name,emp_design,emp_deptt,emp_staffno,patient_name,ref_letter_no,ref_letter_dt,hospno,hospyr,add1,add2,add3,city,pin,adm_date,disch_date,total_charges,rebate,tc_after_rebate,service_charge_perc,servc_charge,total_bill_amt,igst,cgst,sgst,tax_invoice_value,total_adv_received,net_bill_amt,hbillno lotno,to_char(hbilldt,'DD.MM.YYYY') lotdt, regio, stcd3 from wardbill_zbghbildtl where hbilldt between '01-APR-2019' and '30-SEP-2020' order by sap_code, hospno||hospyr")
cur.execute("select sap_code,company_name,customer_name2,emp_name,emp_design,emp_deptt,emp_staffno,patient_name,ref_letter_no,ref_letter_dt,hospno,hospyr,add1,add2,add3,city,pin,adm_date,disch_date,total_charges,rebate,tc_after_rebate,service_charge_perc,servc_charge,total_bill_amt,igst,cgst,sgst,tax_invoice_value,total_adv_received,net_bill_amt,hbillno lotno,to_char(hbilldt,'DD.MM.YYYY') lotdt, regio, stcd3 from wardbill_zbghbildtl where hbilldt = trunc(sysdate-1) order by sap_code, hospno||hospyr")
