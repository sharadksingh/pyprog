...
2. After the BGH_MID_EMPLOYEE is upserted from the MEDICLAIM_MASTER_ENROLL_NEW then the following script has to run.
update bgh_mid_employee a
set a.birth_dt = (select to_date(trim(b.dob_s),'DD/MM/RRRR') from MEDICLAIM_MASTER_ENROLL_AUG17 b where trim(b.minno_s)=a.stno)
where exists(select to_date(trim(b.dob_s),'DD/MM/RRRR') from MEDICLAIM_MASTER_ENROLL_AUG17 b where trim(b.minno_s)=a.stno)

update bgh_mid_employee a
set a.birth_dt = (select to_date(trim(b.dob_e),'DD/MM/RRRR') from MEDICLAIM_MASTER_ENROLL_AUG17 b where trim(b.minno_s)=a.stno)
where exists(select to_date(trim(b.dob_s),'DD/MM/RRRR') from MEDICLAIM_MASTER_ENROLL_AUG17 b where trim(b.minno_s)=a.stno)
'''
