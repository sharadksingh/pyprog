###################################################
# Program Name: monthly_zempdependent.py
###################################################
'''
...
cur  = conn.cursor()
#cur.execute("select nvl(medbkno,' ') medbkno, nvl(name,' ') name, nvl(gender,' ') gender, nvl(dob,' ') dob, nvl(relation,' ') relation, nvl(blood,' ') blood, nvl(depstatus,' ') depstatus, nvl(replace(idmark,' '),' ') idmark, nvl(stno,' ') stno, nvl(empname,' ') empname,  nvl(dor,' ') dor, nvl(gradecd,' ') gradecd, nvl(gradedesc,' ') gradedesc, nvl(deptcd,' ') deptcd, nvl(deptdesc,' ') deptdesc,  nvl(eligibletag,' ') eligibletag from zempdependent order by medbkno")
...
cur  = conn.cursor()
#cur.execute("select nvl(medbkno,' ') medbkno, nvl(name,' ') name, nvl(gender,' ') gender, nvl(dob, ' ') dob, nvl(RelationShip,' ') relation, nvl(BloodGrp,' ') blood, nvl(depstatus,' ') depstatus, nvl(idmark,' ') idmark, nvl(stno,' ') stno,  nvl(empname,' ') empname, nvl(dor, ' ') dor, nvl(gradecd,' ') gradecd, nvl(gradedesc,' ') gradedesc, nvl(DeptCdEdp,' ') deptcd, nvl(DeptNameEdp,' ') deptdesc, 'Y' eligibletag  from REFERRAL_DATA_DEPEN_CURRENT where dor is not null order by medbkno")
...
for row in cur:
#          file.write(row[0]+ '|' + row[1] + '|' + str(row[2]) + '|' + str(row[3]) + '|' + str(row[4]) + '|' + str(row[5]) + '|' + str(row[6]) + '|' + str(row[7]) + '|' + str(row[8]) + '|' + str(row[9]) + '|' + str(row[10]) + '|' + str(row[11]) + '|' + str(row[12]) + '|' + str(row[13]) + '|' + str(row[14]) + '|' + str(row[15]) + '|' + '\n')
          file.write(row[0]+ '|' + row[1].lstrip(" ") + '|' + str(row[2]) + '|' + str(row[3]) + '|' + str(row[4]) + '|' + str(row[5]) + '|' + str(row[6]) + '|' + str(row[7]) + '|' + str(row[8]) + '|' + str(row[9].lstrip(" ")) + '|' + str(row[10]) + '|' + str(row[11]) + '|' + str(row[12]) + '|' + str(row[13]) + '|' + str(row[14]) + '|' + str(row[15]) + '|' + '\n')
file.close()
cur.close()
conn.close()
##############################################################################################################################################
