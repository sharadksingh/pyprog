'''
Program Name: 
This Program generates the Guanator List for the Last Month and szend through mail
This is scheduled to run through the windows scheduler: BGH_BILL_DETAILS_MM
...
#cur.execute("select a.hospno, a.hospyr, a.hospcg, to_char(a.hcatno) hcatno, b.name,
#            to_char(a.hadmdt,'DD/MM/RR') hadmdt, to_char(a.hdisdt,'DD/MM/RR') disdt,
#            a.hpatid, a.hbillno, to_char(a.hbilldt,'DD/MM/RR') billdt, a.hfinyr,
#            to_char(a.bill_grand_total) bill_amount,
#            to_char((a.bill_adm_advance+a.bill_paid_advance)) total_advance, to_char(a.bilttl)final_abill_amount
#            from wardbill_rgstrpat a,wardbill_catedic  b where a.hospcg='E'
#            and a.hfinyr='18-19' and a.hbilldt=trunc(sysdate)-1 and a.hcatno=b.code  order by hospcg, hospno")

cur.execute("select a.staffno, (b.ffname||' '||b.lname) emp_name,b.deptt, d.section,(a.hospno||'/'||a.hospyear) hospno,c.pat_name from adm_edpgrt a,emp_master b,ward_admission_vw  c,emp_dep_code d where a.admdate >= trunc(last_day(sysdate-1), 'mm') and a.admdate <= last_day(trunc(sysdate)-1) and a.staffno = b.stno and b.deptt = d.deptt  and  a.hospno = c.hospno and a.hospyear= c.hospyr  order by a.staffno")
file = open(r"F:\pyora\bgh\adm_bsl_guarantor_list.csv", "w")
...
ws = wb.active
f = open(r'F:\pyora\bgh\adm_bsl_guarantor_list.csv')
reader = csv.reader(f, delimiter=',')
for row in reader:
     ws.append(row)
f.close()
wb.save(r'F:\pyora\bgh\adm_bsl_guarantor_list.xlsx')
...
#   server.set_debuglevel(False) #commenting this out, changing to False will make the script give NO output at all upon successful completion
    ...
    server.quit() #bye bye
    os.remove(r"F:\pyora\bgh\adm_bsl_guarantor_list.csv")
    os.remove(r"F:\pyora\bgh\adm_bsl_guarantor_list.xlsx")
